...
def get_header_text(args, git_hash, cmd):
    header_text = f"""
    This file was autogenerated from the code with convert2ipynb.py script.
    - Main script: `{args.mainpy}`
    - Latest code commit hash: {git_hash}
...
def make_graph(start_file="../scripts/runner.py", basedir=".."):
    ...
    for file, header_lines, lines, deps, remote_import_lines in walk_deps(
        ...
        all_remote_imports.update(dict.fromkeys(remote_import_lines))

        dependencies = list(dict.fromkeys([
                dep for i, (package, dep) in deps.items()
        ]))
        #print("-"*10)
    ...
    G.add_node(index, name="All imports", lines=list(all_remote_imports))
...
if __name__ == "__main__":
    ...
    try:
        ...
        git_hash = "git latest commit's hash not defined, probably we are not not in git repo?"
    cmd = " ".join(sys.argv)

    header_text = get_header_text(args, git_hash, cmd)
